...
| Celovit code + security review (2026-09-17) | ✅ | v1.29 | **Varnost:** shranjeni XSS v JS atributih (`onsubmit="confirm('…{{ x }}')"`) → `data-confirm` + globalni handler; CSP z nonce + SRI za vse CDN vire; `UserValidationMiddleware` (deaktiviran/izbrisan uporabnik takoj odjavljen, sprememba vloge takoj velja); SMTP timeout 30 s + `run_in_threadpool`; admin ne more spremeniti lastne vloge/statusa; TOTP replay zaščita (`totp_zadnji_korak`); bcrypt tudi za neobstoječega uporabnika; produkcija zavrne privzeti SECRET_KEY/ADMIN_GESLO; brisanje skupin samo admin; audit log za reset gesla, skupine, ZRS/uvoz nastavitve. **Baza:** `PRAGMA foreign_keys=ON`, Alembic 010 (čiščenje sirot + `totp_zadnji_korak`), modeli usklajeni z migracijami. **Popravki:** ZRS izvoz s šumniki v oznaki (500), `leto` privzeto ob zahtevi, AKOS URL-kodiranje KZ, LIKE escape v `/clani/iskanje`, značke tipov v seznamu, backup DB prek SQLite backup API. **Odvisnosti:** fastapi 0.141.1, starlette 1.6.0, python-multipart 0.0.32, uvicorn 0.53.0 (9 Dependabot alertov). **Testi:** conftest izoliran od `data/clanstvo.db`, 39 novih testov (205 skupaj), odstranjeni tavtološki asserti |
